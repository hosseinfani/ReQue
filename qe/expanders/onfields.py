...
class QueryExpansionOnFields(RelevanceFeedback):
    def __init__(self, ranker, prels, anserini, index, corpus, replace=False, topn=3, top_n_terms=10):
    ...
    def get_model_name(self):
        return super().get_model_name().replace('topn{}'.format(self.topn),
                                                'topn{}.{}'.format(self.topn, self.top_n_terms))

    def extract_raw_documents(self, docid):
        index_address = self.index
        anserini_address = self.anserini
        cmd = '{}/target/appassembler/bin/IndexUtils -index {} -dumpRawDoc {}'.format(anserini_address, index_address,
                                                                                      docid)
